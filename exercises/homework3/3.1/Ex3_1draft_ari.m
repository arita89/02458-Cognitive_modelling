RGB = imread('beach.jpg');
%J = rgb2gray(RGB);
J = (RGB);
%figure, imagesc(J)
size (J)
I = imcrop(J,[1 1 699 399]); %resizing image to shape 400x600
...
for i = r:-1:1 %40 to 1
    for j = c:-1:1  %1 to 70
        if t > 0
            ...
            patches_hat{i,j} = reshape(vectors_hat, [10,10]); %re-creating patches
            t = t-1;
        end
    end
end
...
xlabel('21 principal components ')
ylabel('Cum Var')
for i1=1:numel(y)

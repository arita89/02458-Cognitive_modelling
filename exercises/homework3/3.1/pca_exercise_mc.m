...
for figure_t = 1:1:size(fig_title,1)
    ...
    n_pca = i;      %to have the S reconstructed with fewer pcas
    %n_pca = 100;   %to have the full S recostructed
    %n_pca = 6;     %only first six
    ...
    % plot
    fig_idx = fig_idx + 1;
    subplot(1,3,fig_idx), imshow(I_project)
    title('pca scene- imshow')
    fig_idx = fig_idx + 1;
    subplot(1,3,fig_idx), imagesc(I_project)
    title('pca scene- imagesc')
    sgtitle('Reconstruction of Image with PCA', fontsize=16)

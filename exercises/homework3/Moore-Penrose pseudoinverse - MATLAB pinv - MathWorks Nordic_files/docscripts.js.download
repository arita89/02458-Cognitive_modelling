...
function adjustTopNavWidth(resetLinks) {
    ...
    if (resetLinks) {
        ...
        inspectTopNavLinks(topNavContainer);
...
function requestLeftNavItems(shortName, pathToDocRoot) {
    ...
    var services = {
      ...
      "webservice":"/help/search/prodfilter/doccenter/en/R2019a"
    }
...
function getDocRelease() {
    ...
    if (localeEl.length === 0) {
        ...
        var dataRelease = localeEl.attr('data-release');
        if (/[Rr]20\d\d[ab]/g.test(className)) {
            return localeEl.attr('data-release');
        }
    } else {
        for (var i = 0; i < localeEl.length; i++) {
            if (localeEl[i].getAttribute('itemprop') === 'content title') {
                var className = localeEl[i].className;
                if (/[Rr]20\d\d[ab]/g.test(className)) {
                    return localeEl[i].className.replace('r', 'R');
                }
            }
        }
    }
